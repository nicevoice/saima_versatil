		...
		<script>
			function delComment(thi ,cid){
				alert('确定删除："'+thi.parentNode.innerText.slice(0, 7)+'..."吗？',null, function(){
					loading(true);
					$.ajax({
						url: "<?php echo U('Forum/delComment',array('token'=>$_GET['token'],'wecha_id'=>$_GET['wecha_id']));?>",
						type:"post",
						data:'cid='+cid,
		...
		<div id="container" class="container" <?php if($bgurl != NULL): ?>style="background:url('<?php echo ($bgurl); ?>') repeat-y center 0"<?php endif; ?>>
						...
						<?php if(is_array($comment)): $i = 0; $__LIST__ = $comment;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$comment): $mod = ($i % 2 );++$i;?><li>
							<dl class="tbox">
								...
								<dd><!-- xx  reply xx -->
									...
									<p><?php echo (htmlspecialchars_decode($comment["content"])); if($comment['uid'] == $_GET['wecha_id']): ?><span class="icons icons_del" onclick="delComment(this,<?php echo ($comment["id"]); ?>);">&nbsp;</span><?php endif; ?></p>
									<time>
								...
								<dd>
									<span class="icons icons_op" onclick="showOperate(true, this, event);">
										<a href="/index.php?g=Wap&m=Forum&a=recomment&cid=<?php echo ($comment["id"]); ?>&tid=<?php echo ($topics["id"]); ?>&reid=<?php echo ($comment["uid"]); ?>&wecha_id=<?php echo ($_GET['wecha_id']); ?>&token=<?php echo ($_GET['token']); ?>" class="btn">回复</a>
			...
			<footer>
				<section class="nav_footer">
					<ul class="box">
						<li>
							<a href="<?php echo U('Forum/index',array('wecha_id'=>$_GET['wecha_id'],'token'=>$_GET['token']));?>">
								<span class="icons_back">&nbsp;</span>
								<label>返回</label>
							</a>
						</li>
						<li>
							<a href="<?php echo U('Forum/commentAdd',array('wecha_id'=>$_GET['wecha_id'],'token'=>$_GET['token'],'tid'=>$topics['id']));?>">
